...
and avalia_cmd amb cmd =
  ...
  | CmdIf (teste, entao, senao) ->
      ...
      | _ -> failwith "avalia_cmd if: valor não booleano"
      )
  ...
  | CmdWhile (teste, corpo) -> 
      ...
      | _ -> failwith "avalia_cmd while: valor não booleano"
      )

  (*| For _ -> failwith "avalia_cmd for: comando não foi implementado"*)
  | CmdAtrib (elem, expr) ->  
     let obtem_nome () = 
       (match elem.S.exp with 
          S.ExpVar v -> (match v with
                       ...
                       | A.VarVetor _ -> 
                           failwith "avalia_cmd: vetor não implementado"
                      )
        | _ -> failwith "avalia_cmd atrib: não é variável"
       )
     in
...
and avalia_exp amb {S.exp} =
  ...
  | S.ExpVar v ->
    (match v with
     ...
     | A.VarVetor _ -> failwith "avalia_exp: vetor não implementado"
    )
  | S.ExpBin (op, esq, dir) ->
...
let interprete ast =
  ...
  try
    ...
    | EntVar _ -> failwith "main não pode ser o nome de uma variável"
  with
    Not_found -> failwith "intereprete: nenhuma função main foi encontrada"
