...
let rec verifica_dup xs =
  ...
  | (nome,_)::xs -> 
    ...
    else failwith ("Parametro duplicado " ^ nome)
         ...
         CmdReturn exp -> 
            (match exp with
                ...
                | Some e -> 
                    ...
                    let _ = tipos_iguais "O tipo retornado e %s mas foi declarado como %s" 
                            exp_tip.tinf tiporet 
                    in CmdReturn (Some exp_tip)
                )
        | CmdIf (teste, entao, senao) ->
            let teste_tip = verifica_exp amb teste in
            let _ = tipos_iguais "O teste do if deveria ser do tipo %s e nao %s" 
            ...
            (match elem with
                ...
                ( match v with 
                  ...
                  ( try (match (Amb.busca amb nome) with
                    ...
                    with Not_found -> failwith ("A variável " ^ nome ^ " nao foi declarada")
                  )
                | A.VarVetor _ -> failwith "verifica_exp: vetor nao implementado"
                )
...
and verifica_exp amb exp =
    match exp with
        ...
        | A.ExpVar v ->
            (match v with
               A.VarSimples nome -> 
                 ...
                 with Not_found -> 
                         failwith ("A variavel " ^ nome ^ " nao foi declarada")
                 )
             | A.VarVetor _ -> failwith "verifica_exp: vetor nao implementado"
            )
        | A.ExpBin (op, esq, dir) ->
          ...
          let verifica_aritmetico () =
            (match esq_tip.tinf with
             ...
             | A.TFloat -> tipos_iguais 
                         "O operando esquerdo e do tipo %s mas o direito e do tipo %s" 
                         esq_tip.tinf dir_tip.tinf 
             | t -> failwith ("um operador aritmetico nao pode ser usado com o tipo " ^
                              (nome_tipo t))
            )
          and verifica_relacional () =
            (match esq_tip.tinf with
             ...
             | A.TString -> tipos_iguais 
                         "O operando esquerdo e do tipo %s mas o direito e do tipo %s" 
                         esq_tip.tinf dir_tip.tinf 
             | t -> failwith ("um operador relacional nao pode ser usado com o tipo " ^
                              (nome_tipo t))
            ) 
          and verifica_logico () =
            (match esq_tip.tinf with
               A.TBool -> tipos_iguais 
                         "O operando esquerdo e do tipo %s mas o direito e do tipo %s" 
                         esq_tip.tinf dir_tip.tinf 
             | t -> failwith ("um operador logico nao pode ser usado com o tipo " ^
                              (nome_tipo t))
            )   
          in
        ...
        | A.ExpUna (op, exp) ->
           ...
           let _ = (match op with
                      A.Not -> tipos_iguais 
                         "O operando e do tipo %s mas deveria ser do tipo %s" 
        ...
        | A.ExpChamada (nome, args) ->
           let rec verifica_tipos ps fs =
              match (ps, fs) with
               (p::ps), (f::fs) -> 
                  let _ = tipos_iguais 
                           "Parametro do tipo %s mas deveria ser do tipo %s" p f in
           ...
           try 
             begin
               ...
               | Amb.EntVar _ -> failwith (nome ^ " e uma variável e não uma funçao")       
             end
           with Not_found -> failwith ("Nao existe a funcao " ^ nome)
    ...
    match asa with
    	  A.DecVar {nome; tipo; valor} ->
      	    let exp_tipada = verifica_exp amb valor in
                let _ = tipos_iguais "Variavel do tipo %s nao pode receber valor do tipo %s"
...
(* 
      ...
      let _ = (tipos_iguais 
          			 "Variável do tipo %s nao pode receber tipo %s"
